...
void compileConstDecls(void) {
  assert("Parsing constant declarations ....");
  while (lookAhead->tokenType == TK_IDENT) { // Lặp nếu còn định danh
    compileConstDecl();
  }
  assert("Constant declarations parsed ....");
}

void compileConstDecl(void) {
  assert("Parsing a constant declaration ....");
  eat(TK_IDENT);        // Xử lý định danh hằng
  eat(SB_EQ);        // Xử lý dấu '='
  compileConstant();    // Phân tích giá trị hằng
  eat(SB_SEMICOLON);    // Xử lý dấu ';'
  assert("Constant declaration parsed ....");
}
...
void compileGroupSt(void) {
  ...
  eat(KW_BEGIN);          // Từ khóa BEGIN
  compileStatements();    // Phân tích danh sách câu lệnh
  eat(KW_END);            // Từ khóa END
